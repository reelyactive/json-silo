...
function JSONSilo(options) {
  ...
  self.app = express();
  self.silo = new siloManager(options);
  ...
  self.app.use('/s', require('./routes/stories'));
...
/**
 * Get the given story as HTML from the database.
 * @param {String} id The story id to search for.
 * @param {String} rootUrl The root URL of the original query.
 * @param {String} queryPath The query path of the original query.
 * @param {function} callback Function to call on completion.
 */
JSONSilo.prototype.getStoryHTML = function(id, rootUrl, queryPath, callback) {
  var self = this;
  ...
  self.silo.find(id, function(story) {
    ...
    prepareStoryHTML(story, function(html) {
...
/**
 * Return the default story.html with updated <head> for SEO & Physical Web.
 * @param {Object} story The given story as JSON.
 * @param {function} callback Function to call on completion.
 */
function prepareStoryHTML(story, callback) {

  // Prepare the title (TODO: move to helper package)
  var title = '';
  var givenName = getSchemaProperty(story, 'schema:givenName');
  var familyName = getSchemaProperty(story, 'schema:familyName');
  var name = getSchemaProperty(story, 'schema:name');
  if(givenName) {
    title += givenName;
  }
  if(familyName) {
  ...
  // Prepare the description (TODO: move to helper package)
  var description = '';
  var desc = getSchemaProperty(story, 'schema:description');
  var jobTitle = getSchemaProperty(story, 'schema:jobTitle');
  var worksFor = getSchemaProperty(story, 'schema:worksFor');
  var model = getSchemaProperty(story, 'schema:model');
  if(desc) {
    description += desc;
  }
  else if(jobTitle && worksFor) {
    description += jobTitle + ' at ' + worksFor;
  }
  else if(jobTitle) {
    description += jobTitle;
  }
  else if(worksFor) {
    description += 'Works for ' + worksFor;
  }
  else if(model) {
    description += 'Model: ' + model;
  }
  else {
    description = "What's inside this bubble?";
  }


  // Read the base HTML
  fs.readFile(__dirname + '/../web/story.html', 'utf8', function(err, base) {
    var html = '';

    // Update title
    var tagIndex = base.search('<title>');
    if(tagIndex >= 0) {
      var startIndex = base.indexOf('>', tagIndex) + 1;
      var stopIndex = base.indexOf('</title>', startIndex);
      html += base.substring(0, startIndex);
      html += title;

      // Update description
      tagIndex = base.search(/(<meta\s*?name\s*?=\s*?"description"\s*?content\s*?=\s*?")/);
      if(tagIndex >= stopIndex) {
        startIndex = base.indexOf('=', base.indexOf('=', tagIndex) + 1) + 1;
        html += base.substring(stopIndex, startIndex);
        stopIndex = base.indexOf('>', startIndex);
        html += '"' + description + '"';
      }
      html += base.substr(stopIndex);
    }
    callback(html);
  });
}
...
/**
 * Return the value of the given schema property in the given story.
 * @param {Object} story The given story as JSON.
 * @param {String} property The given property to search for.
 * @return {String} The property value, null if not present.
 */
function getSchemaProperty(story, property) {
  if(!story ||
     !story.hasOwnProperty('@graph') ||
     (story['@graph'].length < 1) ||
     !story['@graph'][0].hasOwnProperty(property)) {
    return null;
  }
  return story['@graph'][0][property];
}
