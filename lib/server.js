...
function JSONSilo(options) {
  ...
  this.app.post('/add', function(req, res) {
    addJson(self, getRequestParameters(req), function(url) {
      res.redirect('added.html?url=' + url);
    });
  });

  this.app.listen(this.httpPort);
  console.log("json-silo is listening on port " + this.httpPort);
}
...
function addJson(instance, params, callback) {
  ...
  instance.silo.add(json, function(id) {
    ...
    callback(url); // TODO: handle the case where the JSON was not added
  });
}
 ...
 * @param {String} duration The duration expressed as a String.
