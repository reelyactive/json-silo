...
router.route('/')
  .put(
    ...
    function(req, res, next) {
      return replaceKeys(req, res, next);
    }
  );


/**
 * Creates a public/private key & saves them in the root folder
 * as seperate files.
 * @param {Object} res JSON response
 * @param {Object} req JSON request
 * @param {Function} next For the express middleware
 */
function replaceKeys(req, res, next) {
  async.series([
    ...
    // Generate the keys
    function(callback) { keygen(req, res, req.user, callback); },
  ],
 ...
 * UNAUTHORIZED   Password is invalid.
 */
function keygen(req, res, user, callback) {
